...
<body>
  <div style="text-align:center;margin-bottom:16px;" class="no-print">
    ...
    <button class="btn-print" style="background:#6b7280;" onclick="window.close()"><i class="fa-solid fa-xmark"></i> Tutup</button>
  </div>
    ...
    <div class="info">
      ...
      <div class="meta">
        ...
        <p>
          ...
          @if($order->paid_at)Dibayar: {{ $order->paid_at->format('d M Y H:i') }}<br>@endif
          @if($order->activated_at)Diaktifkan: {{ $order->activated_at->format('d M Y H:i') }}<br>@endif
